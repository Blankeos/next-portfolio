...
// next.config.js
const nextConfig = {
  // Always used for `next dev` and `next build`
  webpack: (config) => {
  ...
  // Used for `next dev --turbo` (Ref: https://stackoverflow.com/questions/79329145/how-to-import-svg-icons-in-nextjs-15)
  // If you are using an older version of NextJS. Wrap the rules under "experimental.turbo"
  // i.e. experimental: { turbo: { rules: { ... } } }
  turbopack: {
    rules: {
